...
function isSubmitted($classCode) {
	$c = mysql_query("SELECT grd_sub FROM class WHERE class_code=$classCode");
	$crs = mysql_fetch_row($c);
	return $crs[0]==1;
}
?>
...
<?php
		if(isset($_POST['submit_save_grades'])) {
			...
			$n = count($_POST['idnum']);
			for($i=0; $i<$n; $i++) {
				if(!empty($_POST['mid'][$i])) $mid = $_POST['mid'][$i]; else $mid = '';
				if(!empty($_POST['fin'][$i])) $fin = $_POST['fin'][$i]; else $fin = '';
				if(!empty($_POST['rating'][$i])) $rating= $_POST['rating'][$i]; else $rating= '-';

				$idnum = $_POST['idnum'][$i];
				
				mysql_query("UPDATE sub_enrol SET mgrade='$mid', fgrade='$fin', rating='$rating' WHERE idnum=$idnum AND class_code=$class_code");
			...
			//$date = date('Y-m-d');
			mysql_query("INSERT INTO subm_history (date, remarks, class_code) 
				VALUES ('$date', 'submitted through the Grade Form.', $class_code)");
			mysql_query("UPDATE class SET sub_dt = '$date', grd_sub=0 WHERE class_code=$class_code");	
...
<div>
    ...
    <form method="post" action="">
        <?php $mdl = mysql_query("SELECT * FROM modassgn WHERE modlno=1 AND user='{$_SESSION['user']}'"); ?>
        <?php if(mysql_num_rows($mdl)) { ?>
	    <?php       ListClass("class_code","class_code",
					    "SELECT class_code, name, descript 
						    FROM class, subjects
						    WHERE class.sub_code=subjects.sub_code
						    AND sem_code={$_SESSION['sem_code']}
						    ORDER BY name","","width: 600px;"); ?>
	    <?php } else { ?>
	    <?php       ListClass("class_code","class_code",
					    "SELECT class_code, name, descript 
						    FROM class, subjects
...
<div style="background-color: #fff">
    ...
    <input type="button" value="X" onclick="document.location='index.php?page=grade_form'"
    		style="float:right;" />
    <br />
    
<?php
			$clsrec = mysql_query("SELECT * FROM class_record WHERE class_code={$_POST['class_code']}");
			if(mysql_num_rows($clsrec)>0){
...
<?php
			$stn = mysql_query("SELECT stud_enrol.idnum, lname, fname, mi, cr_acrnm, year, mgrade, fgrade, rating  
			...
			echo mysql_error();
			$n=0;
			while($stnr=mysql_fetch_assoc($stn)) { 	
?>
        <tr>
        	<td>
				<?php echo ++$n . "."; ?>
            ...
            <td><input type="text" id="mid<?php echo $n;?>" name="mid[]" style="width: 35px; border: 0px;"
            		onChange="computeRating(<?php echo $n; ?>, this);"
                    value="<?php if(isset($stnr['mgrade'])) echo $stnr['mgrade'];?>" /></td>
            <td><input type="text" id="fin<?php echo $n;?>" name="fin[]" style="width: 35px; border: 0px;" 
            		onChange="computeRating(<?php echo $n; ?>, this);"
                    value="<?php if(isset($stnr['fgrade'])) echo $stnr['fgrade'];?>" /></td>
            <td><input type="text" id="rt<?php echo $n;?>" name="rating[]" style="width: 35px; border: 0px;" 
            		value="<?php if(isset($stnr['rating'])) 
							if($stnr['rating']=='NULL') echo ''; else echo $stnr['rating'];?>" /></td>
        </tr>
<?php 		} ?>        
		<tr>
        	<td colspan="9" align="right">
        		<?php if(!isSubmitted($_POST['class_code'])) : ?>
            	<input type="submit" name="submit_save_grades" value="Submit"
                	style="font-size: 14pt; width: 150px" />
                <?php else: ?>
                	<p>
