...
if (isset($_POST['submit_create_prospectus'])) {
	if (isset($_REQUEST['prp_code'])) {
		mysqli_query($db, "UPDATE prospectus SET course={$_POST['cr_num']}, year={$_POST['year']}, clg_no={$_SESSION['clg_no']}
						WHERE prp_code={$_REQUEST['prp_code']}");
		if (mysqli_error($db)) echo "<div style='float: none' class='error'>" . mysqli_error($db) . "</div>";
		else echo "<div class='error1' style='float:none'>Prospectus Updated.</div>";
	} else {
		mysqli_query($db, "INSERT INTO prospectus (course, year, clg_no) 
					VALUES ({$_POST['cr_num']}, {$_POST['year']}, {$_SESSION['clg_no']})");

		$lid = mysqli_query($db, "SELECT LAST_INSERT_ID() AS lid");
		$lidr = mysqli_fetch_row($lid);
...
if (isset($_POST['prp_code'])) {
	$pros = mysqli_query($db, "SELECT course as 'cr_num' , year FROM prospectus WHERE prp_code={$_POST['prp_code']}");
...
if (isset($_POST['submit_insert_subject'])) {
	...
	if ($check) echo "<div class='error' style='float:none'>One or more fields are left blank. E. G. $check</div>";
		...
		mysqli_query($db, "INSERT INTO pros_subj (prp_code, sub_code, semyr, c_units) 
					VALUES ({$_REQUEST['prp_code']}, {$_REQUEST['sub_code']}, $semyr,{$_REQUEST['c_units']})");
		if (mysqli_error($db)) echo "<div class='error' style='float:none'>" . mysqli_error($db) . "</div>";
		else echo "<div class='error1' style='float:none'>Subject Inserted to the prospectus</div>";
	}
}
if (isset($_POST['submit_remove_pros_subj'])) {
	mysqli_query($db, "DELETE FROM pros_subj WHERE prp_code={$_REQUEST['prp_code']} AND sub_code={$_REQUEST['sub_code']}");
	if (mysqli_error($db)) echo "<div class='error' style='float:none'>" . mysqli_error($db) . "</div>";
...
?>
	...
	<div style="background:#fff;" id="prospectus">
		<input type="image" src="images/printButton.png" class="noprint" onClick="printThisDiv('printFrame','prospectus');" style="float: right; margin-top: 0px; margin-right: 5px;display:screen;">
		<?php
