...
<?php
$fac = mysql_query("SELECT tch_num, lname,fname,mi 
...
<?php while($facr=mysql_fetch_assoc($fac)) { ?>
<tr>
    ...
    <td style="font-size: 13px;">
    	<?php $sb = mysql_query("SELECT name FROM subjects s, class c
								WHERE c.sub_code=s.sub_code
								AND c.sem_code={$_SESSION['sem_code']}
								AND c.tch_num={$facr['tch_num']}"); ?>
        <?php while($sbr=mysql_fetch_row($sb)) { ?>
        	<?php echo $sbr[0] . ", "; ?>
        <?php } ?>
    </td>
</tr>
