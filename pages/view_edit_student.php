...
if(isset($_GET['view_idnum'])){
	if(isset($_POST['submit_update_student'])){
		...
		if($check) echo "<div class='error'>One or more fields are left empty. E.G. $check.</div>";
			...
			mysql_query("UPDATE stud_info SET 
						idext='$idext',
						lname='$lname',
						fname='$fname',
						mi='$mi',
		...
		$stinfo=mysql_query("SELECT * FROM stud_info WHERE idnum={$_GET['view_idnum']}");
		$strow = mysql_fetch_assoc($stinfo);
  ...
  <table width="670" border="0" cellpadding="2">
    ...
    <tr>
      <td height="49" colspan="4" align="right">
      	<input type="submit" name="submit_update_student" id="submit_update_student" value="Save Changes"
