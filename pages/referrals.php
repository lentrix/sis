<?php include("library/lister.php"); include("library/singles.php"); ?>
...
if(isset($_POST['submit_referral'])) {
    ...
    if(isset($_POST['tardiness'])) $tardiness=1; else $tardiness=0;
    if(isset($_POST['irreg_attn'])) $irreg_attn=1; else $irreg_attn=0;
    if(isset($_POST['emotional'])) $emotional=1; else $emotional=0;
    if(isset($_POST['undr_achv'])) $undr_achv=1; else $undr_achv=0;
    if(isset($_POST['others'])) $others=1; else $others=0;
    
    $desc = $_POST['desc'];
    
    $dt = date('Y-m-d');
    
    mysql_query("INSERT INTO referral (idnum, tch_num, date, tardiness, irreg_attn, emotional, undr_achv, others, referral.desc, status)
            VALUES ($idnum, $tch_num, '$dt', $tardiness, $irreg_attn, $emotional, $undr_achv, $others, '$desc','pending')");
    if(mysql_error()) echo "<div class='error' style='display:block'>" . mysql_error() . "</div>";
    else { echo "<div class='error' style='display:block'>Referral Submitted.</div>"; $_REQUEST['idnum']=-1;}
}
?>
<br />
<div>
    ...
    <div>
        <table width="100%">
            ...
            <tr><td>Select Student:</td><td><?php CreateList("idnum","idnum","name",$stud_sql,""); ?></td></tr>
            <tr>
            ...
            <tr><td colspan=2>Detailed description of the act or incident:</td></tr>
            <tr><td colspan=2><textarea name='desc' style="width:100%;height:150px;font-family:arial, helvetica, sans;"></textarea></td></tr>
            <tr><td colspan=2 align="right"><input type="submit" name="submit_referral" value="Submit Referral" /></td></tr>
            </form>
        </table>
    </div>
</div>
...
<?php
    $ref = mysql_query("SELECT referral.* FROM referral WHERE tch_num={$_SESSION['tch_num']} AND status='pending' ORDER BY date DESC");
    while($rfr = mysql_fetch_assoc($ref)) {
        echo "<tr>
                <td>{$rfr['date']}</td>
...
<?php
    $ref = mysql_query("SELECT referral.* FROM referral WHERE tch_num={$_SESSION['tch_num']} AND status='resolved' ORDER BY date DESC");
    while($rfr = mysql_fetch_assoc($ref)) {
        echo "<tr>
                <td>{$rfr['date']}</td>
                <td>" . getFullName($rfr['idnum']) . "</td>
