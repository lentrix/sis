...
if(isset($_POST['submit_new_subject'])){
	...
	if($ch) echo "<div class='error'>One or more field is left blank. E.g. $ch</div>";
		...
		mysql_query("INSERT INTO subjects (name, descript, acad, clg_no) 
...
<?php
	...
	$subs=mysql_query("SELECT * FROM subjects WHERE descript LIKE '$alphapage%' ORDER BY descript");
	while($subr=mysql_fetch_assoc($subs)){  ?>
  	<tr>
	<td width="105" class="tcel">&nbsp;&nbsp;<a href="index.php?page=view_edit_subject&sub_code=<?php echo $subr['sub_code']?>"
	style="font-size: 10pt; text-decoration: none;"><?php echo $subr['sub_code'];?></a></td>
    <td width="363" class="tcel">&nbsp;&nbsp;<?php echo $subr['descript'];?></td>
  	</tr>
<?php }    ?>
  <tr><td colspan="6" style="background-color:#006; -moz-border-radius: 0 0 10 10;">&nbsp;</td></tr>
</table>
</span>