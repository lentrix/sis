...
<?php
$result = mysql_query("SELECT c.class_code, s.name, s.descript, c.cunits, c.punits
...
<?php while($row=mysql_fetch_assoc($result)) { ?>
		<tr bgcolor="<?php echo $col; ?>">
            ...
            <td>
            	...
            	<input type="text" name="punits" value="<?php echo $row['punits']; ?>" 
            	style="font-size: 12px; text-align:center; width: 40px"
                onmouseup="this.select()"
                onChange="request('library/change_payunits.php?c=<?php echo $row['class_code'];?>&v=' + this.value,'pu_<?php echo $row['class_code'];?>');" />
                </td>
        </tr>
