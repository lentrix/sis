...
if (isset($_POST['submit_fees'])) {
	...
	$affected = mysqli_query($db, "SELECT en.enrol_id FROM stud_enrol en WHERE sem_code={$_SESSION['sem_code']} $where");
	while ($affr = mysqli_fetch_assoc($affected)) {
		$check = mysqli_query($db, "SELECT enrol_id FROM acct_item WHERE enrol_id={$affr['enrol_id']} AND acct_title='$title'");
		if (mysqli_num_rows($check) == 0) {
			mysqli_query($db, "INSERT INTO acct_item (enrol_id, acct_title, amount) VALUES 
					({$affr['enrol_id']},'$title',{$amount})");
		} else {
			if ($_POST['overwrite'] == "on") {
				mysqli_query($db, "UPDATE acct_item SET amount=$amount WHERE enrol_id={$affr['enrol_id']} AND acct_title='$title'");
			}
		}
	}
