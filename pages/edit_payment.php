...
$pmt = mysqli_query($db, "SELECT * FROM payments WHERE orno=" . $_GET['orno']);


$row = mysqli_fetch_assoc($pmt);

$stud = mysqli_query($db, "SELECT CONCAT(lname, ', ', fname, ' ', mi) AS 'name' FROM stud_info WHERE idnum=" . $row['idnum']);
...
<form method="post" action="">
    ...
    <p>
        ...
        <select name="idnum">
            ...
            <?php $studs = mysqli_query($db, "SELECT idnum, CONCAT(lname, ', ', fname, ' ',mi)
                AS 'name' FROM stud_info ORDER BY lname, fname"); ?>
            <?php echo mysqli_error($db); ?>
            <?php while ($stdsrow = mysqli_fetch_assoc($studs)) : ?>
                <option value="<?php echo $stdsrow['idnum']; ?>" <?php if ($row['idnum'] == $stdsrow['idnum']) echo " selected"; ?>><?php echo $stdsrow['name']; ?></option>
            <?php endwhile; ?>
