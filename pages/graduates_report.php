...
if (isset($_POST['submit_poa_auth_num'])) {
    ...
    $has_errors = false;
    for ($i = 0; $i < $n; $i++) {
        mysqli_query($db, "UPDATE courses SET po_auth_num={$po_auth_nums[$i]} 
        WHERE cr_num={$cr_nums[$i]}");
        if (mysqli_error($db)) {
            echo "<div class='error'>" . mysqli_error($db) . "</div>";
            $has_errors = true;
        }
    }
    if (!$has_errors) {
...
if (isset($_POST['confirm_delete_grad'])) {
    mysqli_query($db, "DELETE FROM grad_details WHERE id={$_POST['id']}");
    if (mysqli_error($db)) echo "<div class='error'>" . mysqli_error($db) . "</div>";
...
<div id="students" class="clipable" style="height: auto;">
    <a href="javascript:void(0)" onclick="expandContract(document.getElementById('students'));" style="float: right; text-decoration: none">
        >>>
    </a>
    <h3>Students</h3>
    ...
    <div style="width: 300px; height: 300px; overflow: hidden; float: left; border: 0px;">
        ...
        <?php if (isset($_POST['select_student'])) { ?>
            <?php $std = mysqli_query($db, "SELECT CONCAT(i.lname,', ', i.fname,' ',i.mi) AS 'name', i.idnum, 
            ...
            <?php $strow = mysqli_fetch_assoc($std); ?>
            <form method="post" action="">
    ...
    <div style="width: 380px; float: right; border: 0px; overflow: auto; border-left: 1px solid #888">
        ...
        <?php $grad = mysqli_query($db, "SELECT g.id, CONCAT(i.lname,', ',i.fname,' ', i.mi,' [', c.cr_acrnm,']') AS 'grad' 
        ...
        <ol type="1">
            <?php while ($grow = mysqli_fetch_assoc($grad)) { ?>
                <li onmouseover="document.getElementById('<?php echo $grow['id']; ?>').style.visibility='visible'" onmouseout="document.getElementById('<?php echo $grow['id']; ?>').style.visibility='hidden'" style="font-size: 12px;">
                    <?php echo $grow['grad']; ?>
                    <form method="post" action="" style="display: inline; visibility: hidden" id="<?php echo $grow['id']; ?>">
                        ...
                        <input type="submit" name="delete_grad" value="X" style="font-size: 10px;" />
                    </form>
                </li>
            <?php } ?>
        </ol>
...
<div id="poa" class="clipable">
    <a href="javascript:void(0)" onclick="expandContract(document.getElementById('poa'));" style="float: right; text-decoration: none">
        >>>
    </a>
    ...
    <form method="post" action="">
        <table>
            ...
            <?php $result = mysqli_query($db, "SELECT * FROM courses ORDER BY clg_no, cr_acrnm"); ?>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                ...
                <tr>
                    ...
                    <td width="100">
                        <input type="text" name="po_auth_num[]" value="<?php echo $row['po_auth_num']; ?>" style="width:80px; border: 1px solid #333" />
                    </td>
                </tr>
            <?php } ?>
        </table>
