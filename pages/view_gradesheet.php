...
if(isset($_GET['class_code'])) {
	...
	$cls=mysql_query("SELECT class_code, descript, cunits, CONCAT(fname,' ',mi,' ',lname) as 'name', 
...
<div id="grade_sheet" style="background:#fff;">
	<input type="button" style="width:30px;height:30px;background:url(images/printButton.png);float:right;border:0px;"
    		class="noprint" onclick="printThisDiv('printFrame','grade_sheet');" />
  ...
  <table width="700" border="1" cellspacing="0" cellpadding="2" style="border-collapse: collapse;" align="center">
    <tr>
      <td colspan="10" align="center"><p>Mater Dei College<br>
        Tubigon, Bohol<br>
        <br>
        <strong>GRADE SHEET</strong></p><br /></td>
    </tr>
    <tr>
      <td width="32">&nbsp;</td>
...
<?php
	$stds = mysql_query("SELECT lname, fname, mi, CONCAT(cr_acrnm,'-',year) AS 'cy', mgrade, fgrade, rating
    ...
    echo mysql_error();			
	$n=1;
	while($srow=mysql_fetch_assoc($stds)) {
		if($srow['rating']>3) { 
			$rt=5.0;
			$units=0;
			$rem = "Failed";
...
<div style="background:#fff;">
  ...
  <table style="border-collapse: collapse; width: 100%">
    ...
    <?php $sh = mysql_query("SELECT date, remarks FROM subm_history WHERE class_code=$class_code") ?>
    <?php while($shr = mysql_fetch_assoc($sh)) : ?>
    <tr>
