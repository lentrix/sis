...
if (isset($_POST['submit_new_student'])) {
  ...
  if ($check) {
    ...
    mysqli_query($db, "INSERT INTO stud_info (idnum, idext, lname, fname, mi, addb, addt, addp, 
...
<div id="new_stud" style=" overflow: hidden;">
  ...
  <input type="button" value=">>" onclick="if(this.value=='>>') {
      ...
      <table width="670" border="0" cellpadding="2">
        ...
        <tr>
          <td height="49" colspan="4" align="right">
            <input type="submit" name="submit_new_student" id="submit_new_student" value="Save New Student" style="width: 150px; height: 40px; font-weight: bold;" /></td>
        </tr>
      </table>
    </form>
  </span>
...
if (isset($_POST['submit_search'])) {
  ...
  $sr = mysqli_query($db, "SELECT * FROM stud_info WHERE $field LIKE '%$value%' ORDER BY lname, fname");
  if (mysqli_num_rows($sr) > 0) {  ?>
    <table width="653">
      <tr valign="top">
        <td width='81'>ID Number:</td>
        <td width='199'>Name:</td>
        <td width='200'>Address:</td>
      </tr>
      <?php while ($srow = mysqli_fetch_assoc($sr)) { ?>
        <tr>
          <td><strong>
          ...
          <td><?php echo $srow['addb'] . ", " . $srow['addt'] . ", " . $srow['addp']; ?></td>
        </tr>
      <?php    } ?>
    </table>
<?php }
