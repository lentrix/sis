...
<?php
	if(isset($_POST['forward'])){
		...
		mysql_query("UPDATE class SET grd_sub=1 WHERE class_code IN ({$_POST['class_codes']} 0)");
	}
?>
...
if(isset($_POST['submit_delete_message'])) {
	mysql_query("DELETE FROM messages WHERE idmessages={$_REQUEST['idmessages']}");
...
<div style="background: #fff; margin-top: 30px; min-height: 100px;">
	<?php $mdl = mysql_query("SELECT * FROM modassgn WHERE modlno=3 AND user='{$_SESSION['user']}'"); ?>
    ...
    <?php 
    	$grds = mysql_query("SELECT c.class_code, sb.name, sb.descript, t.lname, t.fname, c.sub_dt 
    ...
    <?php if(mysql_num_rows($grds)>0) : ?>
    	...
    	<table>
    		...
    		<?php while($row=mysql_fetch_object($grds)) : ?>
...
<div style="background: #fff; margin-top: 30px; min-height: 100px;">
    ...
    <span style="display:block; max-height:300px; overflow: auto;">
<?php
$msgs = mysql_query("SELECT * FROM messages WHERE receiver='{$_SESSION['user']}' ORDER BY date_sent DESC");
if(mysql_num_rows($msgs)>0) {
	while($mgrow=mysql_fetch_assoc($msgs)) {
...
if(isset($_POST['submit_message'])) {
	...
	if($check) echo "<div class='error'>One or more required fields are left blank. e.g. $check</div>";
		...
		foreach($_REQUEST['recipient'] as $recipient){
			mysql_query("INSERT INTO messages (sender, receiver, subject, message, date_sent)
...
if(isset($_POST['submit_new_password'])) {
    ...
    if($_POST['pwd1']==$_POST['pwd2']) {
        $user = mysql_query("SELECT user FROM users WHERE user='{$_SESSION['user']}' AND sys_pass=PASSWORD('{$_POST['old']}')");
        if(mysql_num_rows($user)==1){
            mysql_query("UPDATE users SET sys_pass=PASSWORD('{$_POST['pwd1']}') WHERE user='{$_SESSION['user']}'");
...
<div style="background: #fff; margin-top: 30px;">
    ...
    <div style="font-size: 9pt;display:none" id="send_message">
		...
		<table align="center">
        <tr>
        	<td valign="middle">To:</td><td><?php createList("recipient[]","user","user","SELECT user FROM users ORDER BY user",
							 "","width: 300px; height:70px;float:none",1); ?>
		...
		<tr>
        	<td valign="top">Subject:</td><td><input type="text" name="subject" 
            	style="width: 500px;border:1px solid #777;font-size: 9pt; color: #444;" />
            </td>
        </tr>
        <tr>
        	<td>&nbsp;</td>
    ...
    <div id="change_password" style="display:none">
        <form method="post" action="" style="clear:both;">
            <table>
                ...
                <tr><td>Retype New Password:</td><td><input type="password" name="pwd2" /></td></tr>
                <tr><td colspan="2" align="right"><input type="submit" name="submit_new_password" value="Change Password" /></td></tr>
            </table>
        </form>
    </div>
</div>
