...
if (isset($_POST['submit_payment'])) {
    ...
    if (empty($_POST['amount']) || empty($_POST['date']) || empty($_POST['orno'])) {
        ...
        mysqli_query($db, "INSERT INTO payments (orno, sem_code, idnum, name, date, amount, acct_no)
...
$last_orno = mysqli_query($db, "SELECT orno FROM payments ORDER BY orno DESC LIMIT 0, 1");
$lor = mysqli_fetch_row($last_orno);
$next_orno = $lor[0] + 1;
?>
...
<div>
    <form method="post" action="">
        <table style="border-collapse: collapse; max-height: 300px; overflow: auto;">
            ...
            <tr>
                <td><input type="text" name="date" value="<?php echo date('Y-m-d'); ?>" style="width:95px;" /></td>
                <td><input type="text" name="orno" value="<?php echo $next_orno; ?>" style="width: 110px;" /></td>
                <td>
                    <select name="idnum" style="width: 200px;" id="nameSelect">
                        <option value="NULL"></option>
                        <option value="NULL" onClick="changeInput()">Enter name...</option>
                        <?php
                        $result = mysqli_query($db, "SELECT idnum, lname, fname, mi 
            ...
            <?php $date = date('Y-m-d'); ?>
            <?php $pmt = mysqli_query($db, "SELECT p.date, p.orno, 
            ...
            <?php while ($prow = mysqli_fetch_assoc($pmt)) { ?>
                <tr>
