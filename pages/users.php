...
if (isset($_POST['submit_new_user'])) {
	...
	if ($check) echo "<div class='error'>One or more fields are left blank. E.G. $check</div>";
		...
		$usr = mysqli_query($db, "SELECT user FROM users WHERE user='{$_REQUEST['user']}'");
		if (mysqli_num_rows($usr)) {
			...
			if (mysqli_error($db)) echo "<div class='error'>" . mysqli_error($db) . "</div><br />";
			else echo "<div class='error1'>The new users has been added successfully!</div><br />";
		} else {
			mysqli_query($db, "INSERT INTO users (user, fullname, tch_num, clg_no, sys_pass)
	...
	if (isset($_REQUEST['user'])) {
		$usr = mysqli_query($db, "SELECT * FROM users WHERE user='{$_REQUEST['user']}'");
		if (mysqli_num_rows($usr)) {
			$usrow = mysqli_fetch_assoc($usr);
...
if (isset($_POST['submit_confirm_delete_user'])) {
	mysqli_query($db, "DELETE FROM users WHERE user='{$_REQUEST['user']}'");
	...
	<div id="new_user" style="display: none;">
		<form method="post" action="" style="clear:both">
			<table>
				<tr>
					<td>User Name:</td>
					<td><input type="text" name="user" <?php check("user"); ?> /></td>
				</tr>
				<tr>
					<td>Full Name:</td>
					<td><input type="text" name="fullname" <?php check("fullname"); ?> /></td>
				</tr>
				<tr>
					<td>Teacher:</td>
					<td><?php CreateList(
							...
							"tch_num",
							"name",
							"SELECT CONCAT(lname,', ',fname,' ',mi) as 'name', tch_num FROM teacher ORDER BY lname, fname",
							""
						); ?></td>
				</tr>
				<tr>
					...
					<td><?php CreateList(
							...
							"clg_no",
							"college",
							"SELECT clg_no, college FROM colleges ORDER BY college",
							""
						); ?></td>
				</tr>
				<tr>
					<td>Password:</td>
					<td><input type="password" name="sys_pass" <?php check("sys_pass"); ?> /></td>
				</tr>
			</table>
			<input type="submit" name="submit_new_user" value="Submit" style="float: right;" />
...
if (isset($_POST['submit_add_module'])) {
	mysqli_query($db, "INSERT INTO modassgn (user,modlno) VALUES ('{$_REQUEST['user']}',{$_REQUEST['modlno']})");
	if (mysqli_error($db)) echo "<div class='error'>" . mysqli_error($db) . "</div>";
}

if (isset($_POST['submit_remove_module'])) {
	mysqli_query($db, "DELETE FROM modassgn WHERE user='{$_REQUEST['user']}' AND modlno={$_REQUEST['modlno']}");
	if (mysqli_error($db)) echo "<div class='error'>" . mysqli_error($db) . "</div>";
}
?>
...
<div>
	...
	<div>
		...
		<?php if (isset($_REQUEST['user']) && !empty($_REQUEST['user'])) {  ?>
			...
			<form id="module_assignments" action="" method="post" style="display: inline-block;width:200px; border:1px solid #777;padding: 10px;margin-left: 10px;">
			...
			<form id="assigned_modules" action="" method="post" style="display: inline-block;width:200px; border:1px solid #777;padding: 10px;margin-left: 10px;">
				<input type="hidden" name="user" value="<?php echo $_REQUEST['user']; ?>" />
