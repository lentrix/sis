...
<?php 
    if(isset($_POST['submit_confirm_delete_enrol'])) {
        mysql_query("DELETE FROM stud_enrol WHERE idnum={$_POST['idnum']} AND sem_code={$_SESSION['sem_code']}");
    ...
    if(isset($_POST['submit_confirm_withdraw_enrol'])) {
        mysql_query("UPDATE stud_enrol SET en_status='withdrawn' WHERE idnum={$_POST['idnum']} AND sem_code={$_SESSION['sem_code']}");
        mysql_query("UPDATE sub_enrol SET rating='W' WHERE idnum={$_POST['idnum']} AND sem_code={$_SESSION['sem_code']}");
        if(mysql_error()) echo "<div class='error'>" . mysql_error() . "</div>";
        else echo "<div class='error'>Enrolment Record Withdrawn</div>";
    }
    
	if(isset($_REQUEST['idnum'])) { 
		$en = mysql_query("SELECT * FROM stud_enrol WHERE idnum={$_REQUEST['idnum']} AND sem_code={$_SESSION['sem_code']}");
		if(mysql_num_rows($en) && !isset($_POST['submit_enrol'])){
...
<?php //process saving of the stud_enrol...
		if(isset($_POST['submit_enrol'])){
			if(mysql_num_rows($en)==1){
				mysql_query("UPDATE stud_enrol SET course={$_REQUEST['course']}, year='{$_REQUEST['year']}', en_status='{$_REQUEST['en_status']}'
				    WHERE idnum={$_REQUEST['idnum']} AND sem_code={$_SESSION['sem_code']}");
				
			}else{
				mysql_query("INSERT INTO stud_enrol (idnum, sem_code, course, year, en_status)
...
if(isset($_POST['submit_add_student_subject'])){
	...
	$ClassDetails = mysql_query("SELECT * FROM class_time WHERE class_code=$NewClassCode");
	$cdRow = mysql_fetch_assoc($ClassDetails);
...
if(isset($_POST['submit_delete_student_class'])){
	mysql_query("DELETE FROM sub_enrol WHERE class_code={$_POST['class_code']}
			AND idnum={$_POST['idnum']}");
	if(mysql_error()) echo "<div class='error'>Unable to delete! Technical description:<br>" . mysql_error() . "</div>";
...
if(isset($_POST['submit_add_new_class'])) {
	...
	foreach($class_codes as $code){
		$class = mysql_query("SELECT class_code, sub_code FROM class WHERE class_code=$code");
		if(mysql_num_rows($class)>0) {
			$classr = mysql_fetch_assoc($class);
			...
			if($population>=$limit) {
				...
				$time = mysql_query("SELECT * FROM class_time WHERE class_code=$code");
				$timer = mysql_fetch_assoc($time);
				
				if(mysql_error()) echo "<div class='error'>" . mysql_error() . "</div>";
				
				$conflict = checkStudentSubjectTimeConflict($timer['t_start'], $timer['t_end'], $_REQUEST['idnum'], $timer['day']);
				if(!$conflict && mysql_num_rows($time)>1)
					$conflict = checkStudentSubjectTimeConflict($timer['t_start'], $timer['t_end'], $_REQUEST['idnum'], $timer['day']);
				if($conflict) {
					$confr=mysql_fetch_assoc($conflict);
...
<?php
    ...
    $enstatus = mysql_query("SELECT en_status FROM stud_enrol WHERE idnum={$_REQUEST['idnum']} AND sem_code={$_SESSION['sem_code']}");
    if(mysql_num_rows($enstatus)==1){
        $en = mysql_fetch_row($enstatus);
        if($en[0]=="withdrawn"){
		...
		<input type="button" value="Search &amp; Add" style="float: right; margin-top: -8px; margin-right: -8px;" 
			onclick="document.getElementById('add_stud_class').style.display='block'" class="noprint">
		<input type="image" src="images/printButton.png" class="noprint"
				onclick="printThisDiv('printStudyLoadFrame','study_load');"
...
?>		
		<div style="margin-top: 20px; padding-top: 10px; background: #ccc;" id="study_load">
			...
			<?php include("library/stud_subjects.php"); ShowStudentSubjects($_REQUEST['idnum'],$_SESSION['sem_code']); ?>
...
<div id="add_stud_class" 
	style="background: url(images/black_transparent.png); display:none;
		width: 100%; height: 100%; position: fixed; top:0px; left: 0px;">
	<input type="image" src="images/close_button_big.png" 
		style="float: right; clear: both; margin-right: 20px;margin-top:10px;"
		onclick="document.getElementById('add_stud_class').style.display='none'">
    ...
    <div style="height: 100px;width:800px;margin-left:auto;margin-right:auto;margin-top: 10px;background: #ffc;">
        <form method="post" action="" style="width: 500px; margin-left:auto; margin-right:auto">
            ...
            <input type="hidden" name="idnum" value="<?php echo $_REQUEST['idnum'];?>" />
            <input type="submit" name="submit_add_new_class" value="Add"
            		style="float:right; width: 150px; font-size: 12pt;" />
        </form>
    </div>
...
<?php
	$clslst = mysql_query("SELECT class.class_code, class.sub_code, descript, 
			cunits FROM class, subjects 
			WHERE class.sub_code=subjects.sub_code 
			AND sem_code={$_SESSION['sem_code']} 
			ORDER BY descript");
	echo mysql_error();
	while($clsrow=mysql_fetch_assoc($clslst)){
...
<div id="change_stud_class" 
		style="background: url(images/black_transparent.png); display:none;
		width: 100%; height: 100%; position: fixed; top:0px; left: 0px;">
	<input type="image" src="images/close_button_big.png"
			style="float: right; margin-top: 20px; margin-right: 20px;"
			onclick="document.getElementById('change_stud_class').style.display='none'">
