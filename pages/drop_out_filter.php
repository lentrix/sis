...
<form method="get" action="">
    ...
    <select name="sem_code">
        <?php $sems = mysqli_query($db, "SELECT * FROM sems ORDER BY sem_code DESC"); ?>
        <?php while ($sems_row = mysqli_fetch_assoc($sems)) : ?>

            <option value="<?php echo $sems_row['sem_code']; ?>" <?php if (isset($_REQUEST['sem_code'])) : ?> <?php if ($_REQUEST['sem_code'] == $sems_row['sem_code']) : ?> selected <?php endif; ?> <?php endif; ?>>
...
<hr />
<input type="image" src="images/printButton.png" class="noprint" onClick="printThisDiv('print_frame','print_div');" style="float: right;" />
...
<?php if (isset($_REQUEST['show_dropped'])) : ?>
    <div id="print_div">
        ...
        <?php $sem = mysqli_query($db, "SELECT sem FROM sems WHERE sem_code=" . $sem_code); ?>
        <?php $semrs = mysqli_fetch_row($sem); ?>
