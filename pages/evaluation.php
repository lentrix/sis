...
Student:
<?php CreateList("idnum","idnum","name",
		...
		"font-size: 9pt;","font-size:9pt;width:250px;"); ?>
Prospectus:
<?php CreateList("prp_code","prp_code","course_year",
		"SELECT CONCAT(courses.course,' - ',year) AS 'course_year', prp_code FROM prospectus, courses
		WHERE prospectus.course=courses.cr_num
		AND courses.clg_no={$_SESSION['clg_no']}","font-size:9pt","font-size:9pt;width:250px;"); ?>
...
if(isset($_POST['submit_change_credit'])) {
	mysql_query("UPDATE sub_enrol SET sub_code={$_REQUEST['sub_code']} 
...
<div>
    ...
    <div>
    	<br />
        <table border="1">
        	<tr><td width="300">Subject:</td><td width="350">Credit to:</td></tr>
<?php
	$ucs = mysql_query("SELECT class.class_code, sub_enrol.sub_code, descript FROM sub_enrol, subjects, class
	...
	while($ucr = mysql_fetch_assoc($ucs)) {
		...
		createList("sub_code","sub_code","descript",
							...
							"font-size:9pt","font-size:9pt;width:300px");					
