    ...
    <table width="400" border="0" cellpadding="10">
  	...
  	<?php
		if(isset($_POST['login'])){
			...
			$lg = mysql_query("SELECT * FROM users WHERE user='$user' AND sys_pass=PASSWORD('$pwrd')") or die(mysql_error());
			if(mysql_num_rows($lg)==1){
				$lgr = mysql_fetch_assoc($lg);
				$_SESSION['user'] = $lgr['user'];
				$_SESSION['fullname'] = $lgr['fullname'];
				$_SESSION['clg_no'] = $lgr['clg_no'];
				$_SESSION['tch_num'] = $lgr['tch_num'];
				$sem=mysql_query("SELECT MAX(sem_code) FROM sems");
