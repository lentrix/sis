...
<?php
		if(isset($_POST['submit_save_grades'])) {
			...
			$n = count($_POST['idnum']);
			for($i=0; $i<$n; $i++) {
				if(!empty($_POST['mid'][$i])) $mid = $_POST['mid'][$i]; else $mid = 'NULL';
				if(!empty($_POST['fin'][$i])) $fin = $_POST['fin'][$i]; else $fin = 'NULL';
				if(!empty($_POST['rating'][$i])) $rating= $_POST['rating'][$i]; else $rating= 'NULL';
