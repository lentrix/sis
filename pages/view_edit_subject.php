...
if(isset($_POST['submit_update_subject'])){
	...
	$subv = mysql_query("SELECT * FROM subjects WHERE sub_code={$_GET['sub_code']}");
	$subr = mysql_fetch_assoc($subv);
	foreach($subr as $n=>$v){
		$_REQUEST[$n]=$v;
	}
}
if(isset($_POST['submit_confirm_delete_subject'])){
	mysql_query("DELETE FROM subjects WHERE sub_code={$_GET['sub_code']}");
	if(mysql_error()) echo "<div class='error'>Unable to delete. 
		...
		Technical Description:<br/>" . mysql_error(). "</div>";
