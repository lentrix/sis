...
if(!isset($_POST['submit_update_class']) && isset($_GET['class_code'])){
	$cls = mysql_query("SELECT subjects.sub_code, class_code, cunits, punits, class.tch_num,
	...
	$clrow=mysql_fetch_assoc($cls);
	$tm = mysql_query("SELECT * FROM class_time WHERE class_code={$_GET['class_code']}");
	
	foreach($clrow as $n=>$v) $_REQUEST[$n]=$v;
	
	$tmr = mysql_fetch_assoc($tm);
...
if(isset($_POST['submit_confirm_delete_class'])){
	mysql_query("DELETE FROM class WHERE class_code={$_GET['class_code']}");
	if(mysql_error()){
	    if(mysql_errno()==1451) echo "<div class='error'>Sorry! You cannot delete a class that is not empty.</div>";
	    else echo "<div class='error'>MySQL Error! Code: " . mysql_errno() . "</div>";
	}else{
...
if(isset($_POST['submit_update_class'])){
	...
	if($check){
		...
		if(!$checkRT && !$checkTT) {
			mysql_query("UPDATE class SET sub_code=$sub_code, tch_num=$tch_num, clg_no={$_SESSION['clg_no']}, 
			    cunits=$cunits, punits=$punits, sem_code={$_SESSION['sem_code']}, class.limit=$limit 
				WHERE class_code = {$_GET['class_code']}");
			
			mysql_query("UPDATE class_time SET 
			...
			if(mysql_error()) echo "Update: " . $ssql . "<br />" . mysql_error();
...
if($_REQUEST['clg_no']==$_SESSION['clg_no'] || mysql_num_rows($mdl)==1) {
?>
...
<tr><td>Subject:</td>
    ...
    <?php CreateList("sub_code","sub_code","descript",
		"SELECT sub_code, descript FROM subjects ORDER BY descript","font-size:9pt;"); ?>
    </td></tr>
<tr><td>Time:</td><td><?php showTimeInput("start_time1"); ?>
...
<tr><td>&nbsp;</td><td><?php showTimeInput("start_time2"); ?>
  &nbsp;&nbsp;&nbsp;to&nbsp;&nbsp; <?php showTimeInput("end_time2"); ?>
  Day: <input type="text" name="day2" maxlength="3" size="3" <?php check("day2");?> />
...
<?php }else { ?>  
  <a style="cursor:pointer;text-decoration:underline;color:blue"
        id="addtimelink" onclick="showAddTime()">Add Time</a>
  </td></tr>
  <tr>
    <td>&nbsp;</td>
    <td id="newtime"></td>
