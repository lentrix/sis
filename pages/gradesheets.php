...
if(isset($_POST['submit_change_status'])) {
	mysql_query("UPDATE class SET grd_sub=0, sub_dt=NULL WHERE class_code={$_REQUEST['class_code']}");
}
if(isset($_POST['submit_reopen_all'])) {
	mysql_query("UPDATE class SET grd_sub=0 WHERE sem_code={$_SESSION['sem_code']}");
}
if(isset($_POST['submit_close_all'])) {
	mysql_query("UPDATE class SET grd_sub=1 WHERE sem_code={$_SESSION['sem_code']}");
}
...
<?php
$grds = mysql_query("SELECT class_code, descript, CONCAT(lname,', ',fname,' ',mi) as 'fullname', sub_dt FROM class, subjects, teacher
		...
		ORDER BY descript");
$n=1;
while($gr=mysql_fetch_assoc($grds)) {
