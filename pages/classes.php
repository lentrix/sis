...
if ($check) echo "<div class='error'>One or more fields are left blank. E.G. $check.</div>";
	...
	if (isset($_POST['submit_new_class'])) {
		...
		if (!$checkRT && !$checkTT) {
			...
			mysqli_query($db, "INSERT INTO class (sub_code, tch_num, clg_no, cunits, punits, sem_code, class.limit)
				VALUES ($sub_code, $tch_num,
					{$_SESSION['clg_no']}, $cunits, $punits, {$_SESSION['sem_code']}, $limit)");

			if (mysqli_error($db)) echo "<div class='error'>Class Detail: " . mysqli_error($db) . "</div>";

			$newid = mysqli_query($db, "SELECT LAST_INSERT_ID() AS id");
			$newidr = mysqli_fetch_row($newid);

			mysqli_query($db, "INSERT INTO class_time (class_code, t_start, t_end, day, rm_no)
			...
			if (mysqli_error($db)) echo "<div class='error'>Class Time Room: " . mysqli_error($db) . "</div>";
			else {
	...
	<div style="border: 1px solid #777; padding: 10px; display: none;background: #ffc;" id="new_class">
		<form id="form1" name="form1" method="post" action="">
			<table>
				...
				<tr>
					<td>Time:</td>
					<td><span id="time1"><?php showTimeInput("start_time"); ?>
...
<input type="image" src="images/printButton.png" class="noprint" onClick="printThisDiv('printFrame','printdiv');" style="float: right; margin-top: 0px; margin-right: 5px;display:screen;">
		...
		<table style="border-collapse: collapse; width: 100%">
			...
			while ($class_row = mysqli_fetch_assoc($db, $class_list)) {   ?>
				<tr>
					...
					<td class="tcel" style="text-align: center"><?php echo getClassPopulation($class_row['class_code']); ?></td>
					<td class="tcel"><?php echo $class_row['teacher']; ?></td>
				</tr>
			<?php } ?>
			<tr>
				<td colspan="8" style="background-color:#006; -moz-border-radius: 0 0 10 10;">&nbsp;</td>
			</tr>
		</table>
	</div>
</div>