...
function showStudentGrade($idnum, $sem_code){
    ...
    <span style="display:block;">
        ...
        Address:
        <?php
            $addr = mysql_query("SELECT CONCAT(addb,', ',addt,', ',addp) AS 'addr', father, addparents 
                    FROM stud_info WHERE idnum=$idnum");
            $addrr=mysql_fetch_assoc($addr);
            echo $addrr['addr'];
        ?>
    </span>
...
<?php	
    ...
    $wsum = 0;
    $wts = 0;
	while($sgrow=mysql_fetch_assoc($subgrade)){
